...
const Style = styled.a`
  ...
  border: 1px solid ${({ theme }) => theme.button.default.borderColor};
  user-select: none;
  cursor: pointer;
  overflow: hidden;
...
const ButtonCareer = forwardRef((props, ref) => {
  ...
  return (
    <Style className="w-[162px] rounded-[20px]" ref={ref} href={props.href}>
      <span
        className="group flex justify-between items-center py-3 px-8 "
        ref={textRef}
      >
        <Text data-text={props.children}>{props.children}</Text>
        <svg
        className="fill-[#1F2937] group-hover:fill-[#ffffff]"
          width={10.94}
          height={10.94}
          viewBox="0 0 12 12"
          fill="none"
